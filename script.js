document.addEventListener('DOMContentLoaded', function () {
    ...
    fields.forEach(function (field) {
      ...
      if (type === 'url') {
        try {
          ...
          showFieldError(field, getErrorMessage('url', field));
          isValid = false;
          if (!firstInvalidField) firstInvalidField = field;
        }
      }
  ...
  function getErrorMessage(type, field, extras) {
    ...
    const messages = {
      ...
      minlength: label + ' must be at least ' + (extras && extras.minLength) + ' characters.',
      maxlength: label + ' must not exceed ' + (extras && extras.maxLength) + ' characters.'
    };
  ...
  function showFormMessage(form, message, type) {
    ...
    msgEl.className = type === 'error' ? 'form-global-error' : 'form-success-message';
  ...
  if (animatableElements.length > 0 && 'IntersectionObserver' in window) {
    ...
    animatableElements.forEach(function (el, index) {
      if (!el.hasAttribute('data-delay') && !el.hasAttribute('data-animate-delay')) {
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: '830ce3bc-101c-41dd-be60-4e307c0413a4',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || '830ce3bc-101c-41dd-be60-4e307c0413a4';
            ...
            if (mobileToggle && navMenu) {
                ...
                if (phoneHeaderBtn) {
                    phoneHeaderBtn.addEventListener('click', function() {
                        ...
                        // Falls back to 0504433140 placeholder which businessInfoUpdater can replace
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '830ce3bc-101c-41dd-be60-4e307c0413a4';
